...
if (bin.length < 500) {
    console.log("CRITICAL: Binary too short (" + bin.length + "). Re-compile using 'solc --bin' not '--bin-runtime'.");
} else {
    console.log("Initiating Full Creation Deployment (Length: " + bin.length + ")...");
    ...
    while (true) {
        ...
        if (receipt && receipt.contractAddress) {
            ...
            if (code.length > 2) {
                ...
                console.log("STATUS: FAILED. EVM returned empty state.");
            }
            break;
        }
        admin.sleep(1);
