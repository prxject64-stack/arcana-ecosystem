...
import { getAllFilesMatching, readJsonFile, readUtf8File, } from "@nomicfoundation/hardhat-utils/fs";
...
export function isResolvedUserRemapping(remapping) {
    return ("type" in remapping &&
...
export class RemappedNpmPackagesGraphImplementation {
     ...
     * remappings for them.
     */
    #generatedRemappingsIntoNpmFiles = new Map();
    static async create(projectRootPath, remappingsReader = (packageName, packageVersion, packagePath, defaultBehavior) => defaultBehavior(packageName, packageVersion, packagePath)) {
        const projectPackageJson = await readJsonFile(path.join(projectRootPath, "package.json"));
        const resolvedNpmPackage = {
        ...
        return new RemappedNpmPackagesGraphImplementation(resolvedNpmPackage, remappingsReader);
    }
    constructor(hardhatProjectPackage, remappingsReader) {
     ...
     */
    async resolveDependencyByInstallationName(from, installationName) {
        ...
        assertHardhatInvariant(npmPackageDependenciesMap !== undefined, "The npm package must be present in the map");
        // If the dependency already exists with this same installation name we
        // reuse it.
        const existingDependencyNpmPackageByInstallationName = npmPackageDependenciesMap.get(installationName);
        ...
        const dependencyPackageJsonPath = await findDependencyPackageJson(from.rootFsPath, installationName);
        ...
        if (existingDependencyNpmPackageBySourceName !== undefined) {
            const resultOfExistingPackage = {
                ...
                generatedRemapping: this.#generateNpmRemapping(from, installationName, existingDependencyNpmPackageBySourceName),
            };
        ...
        const resultOfNewPackage = {
            ...
            generatedRemapping: this.#generateNpmRemapping(from, installationName, newDependencyNpmPackage),
        };
    ...
    async selectBestUserRemapping(from, directImport) {
        ...
        if (bestUserRemapping.type === UserRemappingType.LOCAL ||
            bestUserRemapping.type === UserRemappingType.NPM) {
            return { success: true, value: bestUserRemapping };
        }
        const result = await this.#resolveNpmUserRemapping(from.package, bestUserRemapping);
        if (!result.success) {
    ...
    async generateRemappingIntoNpmFile(fromNpmPackage, directImport, targetInputSourceName) {
        const remappingsIntoFiles = this.#generatedRemappingsIntoNpmFiles.get(fromNpmPackage);
        assertHardhatInvariant(remappingsIntoFiles !== undefined, "Map of generated remappings should exist");
        const existing = remappingsIntoFiles.get(directImport);
        if (existing !== undefined) {
            assertHardhatInvariant(existing.target === targetInputSourceName, "Trying to generate different remappings for the same direct import into an npm file");
            return existing;
        }
        const remapping = {
    ...
    toJSON() {
        return {
            ...
            packageByInputSourceNameRoot: Object.fromEntries(this.#packageByInputSourceNameRoot.entries()),
            installationMap: Object.fromEntries(Array.from(this.#installationMap.entries()).map(([pkg, dependenciesMap]) => {
                return [
                    pkg.inputSourceNameRoot,
                    Object.fromEntries(dependenciesMap.entries()),
                ];
            })),
            userRemappingsPerPackage: Object.fromEntries(Array.from(this.#userRemappingsPerPackage.entries()).map(([pkg, remappings]) => {
                return [pkg.inputSourceNameRoot, remappings];
            })),
            generatedRemappingsIntoNpmFiles: Object.fromEntries(Array.from(this.#generatedRemappingsIntoNpmFiles.entries()).map(([pkg, remappings]) => {
    ...
    async #readPackageRemappings(npmPackage) {
        const allRemappings = await this.#remappingsReader(npmPackage.name, npmPackage.version, npmPackage.rootFsPath, async (_packageName, _packageVersion, packagePath) => this.#defaultReadPackageRemappings(packagePath));
     ...
     */
    async #defaultReadPackageRemappings(packagePath) {
        const remappingsTxtFiles = await getAllFilesMatching(packagePath, (f) => path.basename(f) === "remappings.txt", (f) => !f.endsWith("node_modules"));
        const results = [];
     ...
     */
    #parseAndDeduplicateRemappings(npmPackage, allRemappings) {
        const remappings = [];
        const errors = [];
        const seen = new Set(); // Track by "context:prefix"
        for (const { remappings: remappingStrings, source } of allRemappings) {
            for (const remappingString of remappingStrings) {
                const result = this.#parseUserRemapping(npmPackage, source, remappingString);
                if (!result.success) {
     ...
     */
    #parseUserRemapping(npmPackage, sourceOfTheRemapping, remappingString) {
