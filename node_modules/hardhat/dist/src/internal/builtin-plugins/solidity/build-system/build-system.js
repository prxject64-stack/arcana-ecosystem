...
import { exists, ensureDir, getAllDirectoriesMatching, getAllFilesMatching, move, readJsonFile, remove, writeJsonFile, writeJsonFileAsStream, writeUtf8File, } from "@nomicfoundation/hardhat-utils/fs";
...
export const SUPPRESSED_WARNINGS = [
    {
        message: "Natspec memory-safe-assembly special comment for inline assembly is deprecated and scheduled for removal. Use the memory-safe block annotation instead.",
...
export class SolidityBuildSystemImplementation {
    ...
    async getCompilationJobs(rootFilePaths, options) {
        ...
        const solcConfigSelector = new SolcConfigSelector(buildProfileName, buildProfile, dependencyGraph);
    ...
    async cleanupArtifacts(rootFilePaths, options = {}) {
        ...
        for (const file of await getAllDirectoriesMatching(artifactsDirectory, (d) => d.endsWith(".sol"))) {
            const relativePath = toForwardSlash(path.relative(artifactsDirectory, file));
        ...
        const artifactPaths = await getAllFilesMatching(artifactsDirectory, (p) => p.endsWith(".json") && // Only consider json files
            // Ignore top level json files
            p.indexOf(path.sep, artifactsDirectory.length + path.sep.length) !== -1, (dir) => dir !== buildInfosDir);
    ...
    #hasCompilationErrors(output) {
        return output.errors?.some((x) => x.severity === "error") ?? false;
    }
    /**
    ...
    #printSolcErrorsAndWarnings(errors) {
        ...
        for (const error of filteredErrors) {
            if (error.severity === "error") {
                const errorMessage = this.#getFormattedInternalCompilerErrorMessage(error) ??
                    ...
                    error.message;
                console.error(errorMessage.replace(/^\w+:/, (t) => chalk.red.bold(t)).trimEnd() +
                    "\n");
            }
            else {
        ...
        if (hasConsoleErrors) {
            console.error(chalk.red(`The console.log call you made isn't supported. See https://hardhat.org/console-log for the list of supported methods.`));
            console.log();
        }
    }
    #shouldSuppressWarning(error) {
        const msg = error.formattedMessage ?? error.message;
        return shouldSuppressWarning(msg, this.#options.solidityTestsPath, this.#options.projectRoot);
    }
